...
async function moveToTrash(id){   const idx = state.students.findIndex(s=>s.id===id);   if(idx===-1) return;   const [r] = state.students.splice(idx,1);   r._removedAt = new Date().toISOString();   r.status = 'Removed'; // mark removed so cloud knows    // update local state   state.trash.push(r);   save();   renderAll();    // push change to Firestore (if available)   if(firebaseReady && currentUser){     try{       // update the student's doc in cloud with removed status and timestamp       await saveStudentToFirestore(r);       console.log('Moved to trash in Firestore:', r.id);     }catch(e){       console.warn('Failed to update Firestore when moving to trash', e);     }   } }
function openTrash(){   const modal = document.createElement('div'); modal.className = 'modal-backdrop';   modal.innerHTML = `<div class="modal"><h3>Trash</h3><div id="trashContent" style="max-height:400px;overflow:auto;margin-top:8px"></div><div style="text-align:right;margin-top:12px"><button class="btn ghost" id="closeT">Close</button></div></div>`;   document.body.appendChild(modal);   const box = modal.querySelector('#trashContent');    if(!state.trash || state.trash.length === 0){     box.innerHTML = '<div style="color:var(--muted)">Empty</div>';   } else {     state.trash.forEach(t => {       const el = document.createElement('div');       el.style.padding = '8px';       el.style.borderBottom = '1px solid #f4f6fb';       el.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:center">         <div><strong>${escapeHtml(t.name)}</strong><div style="color:var(--muted);font-size:13px">Removed: ${t._removedAt?.slice(0,10)||''}</div></div>         <div><button class="btn" data-r="${t.id}">Restore</button> <button class="btn ghost" data-d="${t.id}">Delete</button></div>       </div>`;       box.appendChild(el);     });      // Restore handlers     box.querySelectorAll('button[data-r]').forEach(b => {       b.onclick = async () => {         const id = b.dataset.r;         const i = state.trash.findIndex(x => x.id === id);         if(i === -1) return;         const [it] = state.trash.splice(i,1);         // update local item         delete it._removedAt;         it.status = 'Active';         state.students.push(it);         save();         renderAll();          // update cloud         if(firebaseReady && currentUser){           try{             await saveStudentToFirestore(it);             console.log('Restored student in Firestore:', it.id);           }catch(e){             console.warn('Failed to restore student in Firestore', e);           }         }          // refresh modal UI         modal.remove();         openTrash();       };     });      // Permanent delete handlers     box.querySelectorAll('button[data-d]').forEach(b => {       b.onclick = async () => {         if(!confirm('Delete permanently? This will remove the student from the cloud too (if signed in).')) return;         const id = b.dataset.d;         const i = state.trash.findIndex(x => x.id === id);         if(i === -1) return;          const [it] = state.trash.splice(i,1); // remove from local trash         save();         renderAll();          // delete from Firestore if possible         if(firebaseReady && currentUser){           try{             await db.collection('students').doc(String(id)).delete();             console.log('Deleted student doc from Firestore:', id);           }catch(e){             console.warn('Failed to delete student doc from Firestore', e);           }         }          // refresh modal UI         modal.remove();         openTrash();       };     });   }    modal.querySelector('#closeT').onclick = () => modal.remove(); }
